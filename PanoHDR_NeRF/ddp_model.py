...
def lift_gaussian(ray_o, ray_d, base_radius, mu, hw):
    ...
    small_value[::] = 1e-10
    d_mag_sq = torch.max(small_value, torch.sum(d_outer_diag, dim=-1, keepdim=True))       # [..., 3]
...
class NerfNet(nn.Module):
    def __init__(self, args):
        ...
        self.fg_net = MLPNet(D=args.netdepth, W=args.netwidth,
                             ...
                             use_viewdirs=args.use_viewdirs,is_HDR=args.is_HDR)
    ...
    def forward(self, ray_o, ray_d, fg_z_max, fg_z_vals, bg_z_vals, radii=None, hw=None):
        ...
        fg_viewdirs = viewdirs.unsqueeze(-2).expand(dots_sh + [N_samples, 3])  # [..., N_samples, 3]
        ...
        if not self.ipe:
            ...
            mean, cov_diag = lift_gaussian(ray_o, ray_d, radii*self.radius_multiplier, fg_z_vals, hw)
