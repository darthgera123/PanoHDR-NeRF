...
def resample_depth(weights, depth_mid, N_samples, det=False):
    weights_pad = torch.cat([   #[batch_size, N_samples+2]
            weights[..., :1],
            weights,
            weights[..., -1:],
        ],
                                        dim=-1)
    weights_max = torch.max(weights_pad[..., :-1], weights_pad[..., 1:]) #[batch_size, N_samples+1]
    weights_blur = 0.5 * (weights_max[..., :-1] + weights_max[..., 1:]) #[batch_size, N_samples]

    # Add in a constant (the sampling function will renormalize the PDF).
    resample_padding = 0.01
    weights = weights_blur + resample_padding
    weights = weights[..., 1:-1]
    depth_samples = sample_pdf(bins=depth_mid, weights=weights,
                                N_samples=N_samples, det=det)    # [..., N_samples]
    ...
    for s in range(len(ray_batch_split['ray_d'])):
        ...
        for m in range(models['cascade_level']):
            ...
            if m == 0:
                ...
                if models['alt_sampling']:
                    fg_depth = resample_depth(fg_weights, fg_depth_mid, N_samples+1, det=True)
                else:
                ...
                if models['alt_sampling']:
                    bg_depth = resample_depth(bg_weights, bg_depth_mid, N_samples, det=True)
                else:
...
def log_view_to_tb(writer, global_step, log_data, gt_img, mask, prefix='', is_HDR = False):
    if is_HDR:
        compreseed_gt = gt_img ** (1 / 2.2)
        rgb_im = torch.clamp(img_HWC2CHW(torch.from_numpy(compreseed_gt)), min=0., max=1.)
        writer.add_image(prefix + 'rgb_gt_compressed', rgb_im, global_step)
        rgb_im = torch.clamp(img_HWC2CHW(torch.from_numpy(compreseed_gt / 10.0)), min=0., max=1.)  # just in case diffuse+specular>1
        writer.add_image(prefix + 'rgb_gt_compressed_divided_10', rgb_im, global_step)
    else:
        rgb_im = img_HWC2CHW(torch.from_numpy(gt_img))
        writer.add_image(prefix + 'rgb_gt', rgb_im, global_step)
   
    if mask is not None: 
        mask_im = img_HWC2CHW(torch.from_numpy(mask))
        writer.add_image(prefix + 'mask', mask_im, global_step)
    ...
    for m in range(len(log_data)):
        if is_HDR:
            ...
            writer.add_image(prefix + 'level_{}/rgb_compressed'.format(m), rgb_im, global_step)

            rgb_im = img_HWC2CHW(log_data[m]['rgb'])
            rgb_im = torch.clamp(rgb_im / 10.0, min=0., max=1.)  # just in case diffuse+specular>1
            writer.add_image(prefix + 'level_{}/rgb_compressed_divided_10'.format(m), rgb_im, global_step)
        else:
            rgb_im = img_HWC2CHW(log_data[m]['rgb'])
            rgb_im = torch.clamp(rgb_im, min=0., max=1.)  # just in case diffuse+specular>1
...
def create_nerf(rank, args):
    ...
    models['alt_sampling'] = args.alt_sampling
    if args.single_mlp:
        models['N_MLP'] = 1
    else:
...
def ddp_train_nerf(rank, args):
    ...
    for global_step in range(start+1, start+1+args.N_iters):
        ...
        i = np.random.randint(low=0, high=len(ray_samplers))
        ray_batch = ray_samplers[i].random_sample(args.N_rand, center_crop=False, sphere_sample=args.sphere_sample)
        for key in ray_batch:
            if torch.is_tensor(ray_batch[key]):
        ...
        losses = []                                    # losses on different cascade levels
        for m in range(models['cascade_level']):
            if models['N_MLP'] == 1:
                net = models['net_0']
            ...
            if m == 0:
                ...
                fg_depth_mid = .5 * (fg_depth[..., 1:] + fg_depth[..., :-1])    # [..., N_samples-1]
                if models['alt_sampling']:
                    fg_depth_samples = resample_depth(fg_weights, fg_depth_mid, N_samples+1, det=False)
                    fg_depth, _ = torch.sort(fg_depth_samples)
                else:
                    fg_weights = fg_weights[..., 1:-1]                              # [..., N_samples-2]
                    fg_depth_samples = sample_pdf(bins=fg_depth_mid, weights=fg_weights,
                ...
                bg_depth_mid = .5 * (bg_depth[..., 1:] + bg_depth[..., :-1])
                if models['alt_sampling']:
                    bg_depth_samples = resample_depth(bg_weights, bg_depth_mid, N_samples, det=False)
                    bg_depth, _ = torch.sort(bg_depth_samples)
                else:
                    bg_weights = bg_weights[..., 1:-1]                              # [..., N_samples-2]
                    bg_depth_samples = sample_pdf(bins=bg_depth_mid, weights=bg_weights,
            ...
            if 'autoexpo' in ret:
                ...
                if args.is_HDR:
                    rgb_loss = img2mse(ret['rgb'], rgb_gt ** (1 / 2.2), mask=mask_gt)
                else:
            ...
            if models['N_MLP'] == 1:
                if m == (models['cascade_level'] -1):
                    loss = sum((sum(losses[:-1])*args.coarse_loss_mult, losses[-1]))
                    loss.backward()
                    optim.step()
            else:
                loss.backward()
        ...
        if global_step % args.i_img == 0 or global_step == start+1:
            ...
            if rank == 0:    # only main process should do this
                ...
                log_view_to_tb(writer, global_step, log_data, gt_img=val_ray_samplers[idx].get_img(), mask=val_ray_samplers[idx].get_mask(), prefix='val/', is_HDR = args.is_HDR)
            ...
            if rank == 0:   # only main process should do this
                ...
                log_view_to_tb(writer, global_step, log_data, gt_img=ray_samplers[idx].get_img(), mask=ray_samplers[idx].get_mask(), prefix='train/', is_HDR = args.is_HDR)
...
def config_parser():
    ...
    parser.add_argument("--radius_multiplier", type=float, default=1,
                        help='multiplies the radius of the cones casted when using IPE')
    parser.add_argument("--single_mlp", action='store_true', help='use the same MLP for coarse and fine sampling')
    parser.add_argument("--alt_sampling", action='store_true', help='use the sampling method used in the mip-NeRF paper')
    parser.add_argument("--coarse_loss_mult", type=float, default=0.1,
                        help='How much to downweight the coarse loss(es)')
    parser.add_argument("--sphere_sample", action='store_true', 
                        help='sample rays uniformly in 3D Cartesian coordinates when training on panos')
    # logging/saving options
    parser.add_argument("--i_print", type=int, default=100, help='frequency of console printout and metric loggin')
    parser.add_argument("--i_img", type=int, default=500, help='frequency of tensorboard image logging')
    parser.add_argument("--i_weights", type=int, default=10000, help='frequency of weight ckpt saving')
    parser.add_argument("--saveEXR", action='store_true', help='save output images in EXR format')
    parser.add_argument("--is_HDR", action='store_true', help='If the input images are HDR')

    parser.add_argument("--is_perspective", action='store_true', help='If the camera model is perspective')
    return parser
